...
static function X2AbilityTemplate TargetedBuff(name DataName, string IconImage, optional bool bCrossClassEligible = false, optional X2Effect Effect = none, optional int ShotHUDPriority = default.AUTO_PRIORITY, optional EActionPointCost Cost = eCost_Single, optional EPerkBuffCategory BuffCat = ePerkBuff_Bonus)
{
	local X2AbilityTemplate                 Template;	
	...
	{
		if (X2Effect_Persistent(Effect) != none)
			X2Effect_Persistent(Effect).SetDisplayInfo(BuffCat, Template.LocFriendlyName, Template.LocLongDescription, Template.IconImage, true, , Template.AbilitySourceName);
...
static function X2AbilityTemplate SquadPassive(name DataName, string IconImage, bool bCrossClassEligible, X2Effect_Persistent Effect, optional EPerkBuffCategory BuffCat = ePerkBuff_Bonus)
{
	...
	TriggerTemplate = TargetedBuff(name(DataName $ "_OnUnitBeginPlay"), IconImage, false, Effect,, eCost_None, BuffCat);
...
// Changes the effect as passive in UI
static function SHowIconAsPassive(X2AbilityTemplate Template)
{
	local X2Effect Effect;
	...
	foreach Template.AbilityTargetEffects(Effect)
	{
		if (X2Effect_Persistent(Effect) != none)
		{
			X2Effect_Persistent(Effect).SetDisplayInfo(ePerkBuff_Passive, Template.LocFriendlyName, Template.LocHelpText, Template.IconImage, true, , Template.AbilitySourceName);
		}
	}
}
...
static function AddSecondaryAbility(X2AbilityTemplate Template, X2AbilityTemplate SecondaryTemplate, optional bool UseFirstTemplateLocalizationForSecondTemplate = true)
{
	local X2Effect Effect;
	local X2Effect_Persistent PersistentEffect;
