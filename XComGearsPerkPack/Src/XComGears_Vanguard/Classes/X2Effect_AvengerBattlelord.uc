class X2Effect_AvengerBattlelord extends X2Effect_Battlelord;
...
function RegisterForEvents(XComGameState_Effect EffectGameState)
{
	...
	EventMan.RegisterForEvent(EffectObj, 'UnitTakeEffectDamage', AvengerBattlelord, ELD_OnStateSubmitted,,,, EffectGameState);
	//EventMan.RegisterForEvent(EffectObj, 'AvengerFlyover', EffectGameState.TriggerAbilityFlyover, ELD_OnStateSubmitted,, UnitState);
}




 static function EventListenerReturn AvengerBattlelord(Object EventData, Object EventSource, XComGameState GameState, Name EventID, Object CallbackData)
{
	local XComGameState_Unit TargetUnit, SourceUnit;
	local XComGameStateContext_Ability AbilityContext;
	local XComGameState_Ability FlyoverAbilityState;
		...
		if (SourceUnit.IsFriendlyUnit(TargetUnit) || SourceUnit.ObjectID == TargetUnit.ObjectID)
		{
			...
			if (TacticalRules.VisibilityMgr.GetVisibilityInfo(SourceUnit.ObjectID, TargetUnit.ObjectID, VisInfo))
			{
				if (VisInfo.bClearLOS)
				{
					...
					`XEVENTMGR.TriggerEvent('AvengerFlyover', FlyoverAbilityState, SourceUnit, NewGameState);
